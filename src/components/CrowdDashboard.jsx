...
export default function CrowdDashboard() {
  ...
  useEffect(() => {
    ...
    const generateReport = async () => {
      try {
        ...
        if (isMounted) setReportText(`[SYSTEM ERROR] Failed to generate AI report: ${error.message}`);
      }
    };
  ...
  return (
    <div className="glass-panel animate-fade-in" style={{ height: '100%', padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto' }}>
      <div>
        ...
        <div className="glass-card" style={{ padding: '20px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ 
            ...
            <span className={isTyping ? 'typewriter-cursor' : ''} style={{ whiteSpace: 'pre-wrap' }}>
              {displayedText || 'Awaiting metrics to generate AI scenario...'}
            </span>
          </div>
        </div>
      </div>
...
function MetricCard({ title, value, icon, color }) {
  return (
    <div className="glass-card" style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
      <div style={{ padding: '12px', background: `color-mix(in srgb, ${color} 20%, transparent)`, borderRadius: '12px', color }}>
        {icon}
      </div>
      <div>
